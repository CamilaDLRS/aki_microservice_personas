...
function validate(schema: any) {
  return (req: any, _res: any, next: any) => {
    ...
    if (!r.success) {
      const details = r.error.issues.map((i: any) => `${i.path.join('.')}: ${i.message}`);
...
router.put('/:classId', validate(updateSchema), (req, res, next) =>
...
router.post('/:classId/students', validate(linkStudentSchema), (req, res, next) =>
...
router.post('/:classId/teachers', validate(linkTeacherSchema), (req, res, next) =>
  controller.addTeacher(req, res, next)
);
